...
- `POST /api/projects/tts-workbench/emotion-annotation-analysis` with split turns and `projectId` returns annotated `turns` containing `speaker` and marked-up `text`, and persists the styled text to `audiobook_speech_segment.styled_text` on the matching script preview rows.
